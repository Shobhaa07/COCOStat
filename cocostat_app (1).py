...
T = {
    ...
    "si": {
        ...
        "desc": "මෙම පද්ධතිය පොල් මිල වෙනස්වීම්, ඉල්ලුම් හැසිරීම සහ ඉදිරි මිල අනාවැකි [...]",
        "lang_label": "🌐 භාෂාව",
        "lang_option": "English",
        ...
        "policy_title": "දැනට රජය කුම��් කළ යුතුද?",
...
if "📊 Overview" in section or "📊 දළ" in section:
    ...
    for i, col in enumerate([col1, col2, col3]):
        with col:
            st.markdown(f"""
            <div style='background:{regime_bgs[i]}; border:2px solid {regime_colors[i]}; border-radius:16px; padding:24px; text-align:center;'>
    ...
    with col1:
        ...
        fig_bar.update_layout(height=280, plot_bgcolor="#f8fafc", paper_bgcolor="white")
    ...
    fig_fore = go.Figure()
    fig_fore.add_trace(go.Scatter(x=hist_recent["date"], y=hist_recent["price"], line=dict(color="#3b82f6", width=2.5), name=t["forecast_hist_label"]))
    fig_fore.add_trace(go.Scatter(x=forecast_df["date"], y=forecast_df["price"], line=dict(color="#f59e0b", width=2.5, dash="dash"), name=t["forecast_pred_label"]))
    fig_fore.update_layout(height=340, plot_bgcolor="#f8fafc", paper_bgcolor="white")
    ...
    for i, col in enumerate([col1, col2, col3]):
        with col:
            ...
            <div style='border-radius:16px; overflow:hidden; box-shadow:0 4px 16px rgba(0,0,0,0.08);'>
                <div style='background:{regime_colors[i]}; padding:14px 18px;'><span style='font-weight:800; color:white;'>{t["policy_markets"][i]}</span></div>
                <div style='padding:16px 18px; background:#f8fafc;'><p style='margin:0 0 12px;'>{t["policy_actions"][i]}</p></div>
            </div>
            """, unsafe_allow_html=True)
    ...
    fig_hist = go.Figure()
    fig_hist.add_trace(go.Scatter(x=history_df["date"], y=history_df["price"], fill="tozeroy", fillcolor="rgba(22,163,74,0.1)", line=dict(color="#16a34a", width=1.8)))
    fig_hist.add_hline(y=65, line_dash="dash", line_color="#eab308")
    fig_hist.add_hline(y=80, line_dash="dash", line_color="#ef4444")
    fig_hist.update_layout(height=360, plot_bgcolor="#f8fafc", paper_bgcolor="white")
    st.plotly_chart(fig_hist, use_container_width=True)
    
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("📈 Max", f"Rs. {history_df['price'].max():.2f}")
    c2.metric("📉 Min", f"Rs. {history_df['price'].min():.2f}")
    c3.metric("📊 Avg", f"Rs. {history_df['price'].mean():.2f}")
    c4.metric("📐 StdDev", f"Rs. {history_df['price'].std():.2f}")
    ...
    for i, (col, step) in enumerate(zip(cols, t["method_steps"])):
        with col:
            st.markdown(f"""
            <div style='text-align:center; background:#f8fafc; border-radius:16px; padding:20px; border:1px solid #e2e8f0;'>
                <div style='font-size:2rem; margin-bottom:10px;'>{"📚🔍📏🔮"[i]}</div>
                <div style='font-size:0.88rem; color:#475569;'>{step}</div>
            </div>
            """, unsafe_allow_html=True)
